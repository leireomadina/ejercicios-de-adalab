...
function getBoxModel(bool, width, padding, border) {
  ...
  if (bool === true) {
    ...
    const result = `El ancho del contenido es ${width} y el ancho total de la caja es: ${contentModelWidth}`;
    return result;
  } else {
    // content-box to border-box
    const borderModelWidth = width - padding * 2 - border * 2;
    const result2 = `El ancho del contenido es ${width} y el ancho total de la caja es: ${borderModelWidth}`;
    return result2;
  }
}
